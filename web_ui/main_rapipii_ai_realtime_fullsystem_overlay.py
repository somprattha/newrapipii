...
def edge_fallback(text):
    ...
    return gtts_fallback(text)  # Replace with actual Edge TTS call if available

# -----------------------------
# MAIN SPEAK FUNCTION (AUTO FALLBACK)
# -----------------------------
...
def run_webui():
    try:
        print("🌐 Starting WebUI (localhost:8123)...")
        subprocess.Popen(["python", "start_webui.py"])
    except Exception as e:
        print(f"❌ Failed to start WebUI: {e}")

def run_stt():
    try:
        print("🎙️ Starting Speech-to-Gemini...")
        subprocess.run(["python", "speech_to_gemini.py"])
    except Exception as e:
        print(f"❌ Failed to run STT module: {e}")


def run_overlay_ws():
    try:
        print("🌀 Starting Overlay WebSocket Server...")
        subprocess.Popen(["python", "overlay_websocket_server.py"])
    except Exception as e:
        print(f"❌ Failed to start Overlay WebSocket Server: {e}")


if __name__ == "__main__":
    threading.Thread(target=run_overlay_ws).start()

    threading.Thread(target=run_webui).start()
    time.sleep(3)  # wait a bit for WebUI to boot
    run_stt()
