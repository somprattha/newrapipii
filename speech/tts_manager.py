...
def play_audio(filepath):
    try:
        ...
        from tts_fallback import speak_with_fallback  # ถ้ามีระบบ fallback หลายชั้น

def speak_text_auto_fallback(text, voice_style=None):
    print(f"🔊 พูดด้วยเสียง: {voice_style or 'default'} | ข้อความ: {text}")
    speak_with_fallback(text, voice_style)
